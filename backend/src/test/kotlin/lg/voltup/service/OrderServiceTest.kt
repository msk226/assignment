...
class OrderServiceTest
    ...
    constructor(
        ...
        fun createOrder_shouldThrowExceptionWhenProductNotActive() {
            ...
            val exception =
                assertThrows<ProductNotAvailableException> {
                    orderService.createOrder(testUser.id, request)
                }
        ...
        fun createOrder_shouldUseExpiringPointsFirst() {
            ...
            // 나중에 만료되는 포인트는 사용되지 않음 (setUp의 30일 포인트가 더 먼저 만료됨)
            val usedLaterExpiring = pointRepository.findById(laterExpiringPoint.id).get()
            assertEquals(0, usedLaterExpiring.usedAmount) // 미사용
        }
